...
BR-248 이후 단일 파일 payload는 zip/base64가 아니라 UTF-8 source-text로 저장하고, BR-255 이후 실행 시 readable comment block을 `EMBEDDED_TEXT_FILES`로 복원함.
BR-253 이후 파일 상단에 payload 역할 index를 노출하고, `--single-list-payload` 및 `--single-extract-source`로 내부 구조를 확인할 수 있음.
BR-255 이후 payload는 compact JSON blob이 아니라 파일 아래쪽 `# region Embedded readable source payload` 안의 읽을 수 있는 `#|` source comment block으로 저장함.
...
- source-text payload는 readable comment block container로 저장하고, necessity audit을 통과한 11개 파일만 포함함.
- 줄 수보다 전달 검토 가능성을 우선하여 `# pvdiag_payload_file` metadata와 `#|` 원문 source line을 그대로 노출함.
- payload 구조가 숨겨지지 않도록 `PAYLOAD_FILE_INDEX`와 source extraction path를 제공함.
- 큰 readable payload block은 VS Code에서 접을 수 있는 region으로 분리함.
