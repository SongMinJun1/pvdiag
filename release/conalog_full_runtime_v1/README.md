...
BR-254 이후 큰 payload blob은 본문을 가리지 않도록 파일 아래쪽 `# region Embedded source payload` foldable region에 둠.
...
- 큰 `EMBEDDED_TEXT_JSON_CHUNKS` 블록은 VS Code에서 접을 수 있는 region으로 분리함.
