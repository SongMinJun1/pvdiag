...
## 2. 현재 포함 범위
- stable 범위에는 conalog stable handoff pack, stable output 계약, runtime foundation 문서와 config, current frozen integrated result snapshot 이 포함됨.
...
## 4. 최종 release 절차
- 현재 frozen public baseline 은 `project-main-freeze-v9` 기준으로 읽어야 함.
...
- 외부 전달 시에는 `package/stable_handoff/` 를 1차 진입점으로 안내하고, runtime 및 one-click 문서는 운영 보조 자료로 함께 전달하여야 함.
