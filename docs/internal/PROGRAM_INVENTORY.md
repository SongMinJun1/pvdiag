...
| `research/prognostics/run_paper_pack.sh` | paper pack 생성 진입점 | 지원 도구, `plot_fault_cases.py` 호출 |
...
| `research/prognostics/plot_fault_cases.py` | 현재형 fault case plot 생성 | `run_paper_pack.sh`에서 직접 호출 |
...
| 파일 | 2차 검토 이유 |
|---|---|
| `research/prognostics/plot_fault_cases.py` | `run_paper_pack.sh`에서 직접 호출되므로 실제 입력/출력 계약 확인 필요 |
...
| `research/archive/prognostics/make_fault_case_plots.py` | `research/prognostics/make_fault_case_plots.py` | `plot_fault_cases.py`와 기능 중복, active caller 없음 |
