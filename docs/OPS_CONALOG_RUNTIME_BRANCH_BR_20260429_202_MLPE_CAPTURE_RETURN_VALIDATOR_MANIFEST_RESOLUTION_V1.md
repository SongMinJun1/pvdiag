# OPS_CONALOG_RUNTIME_BRANCH_BR_20260429_202_MLPE_CAPTURE_RETURN_VALIDATOR_MANIFEST_RESOLUTION_V1

## Summary
- BR-111 `mlpe_field_trial_capture_return_validator` already failed closed for unsafe user-filled defaults.
- This patch adds optional manifest resolution for the returned-capture CSV and records input-source provenance.
- It does not change validation buckets, returned-ready logic, truth-intake authorization, threshold authorization, engine authorization, or operator-facing behavior.
...
- Not changed:
  ...
  - returned capture validation buckets
  - returned-ready / waiting / validation-failed logic
...
- The dedicated smoke verifies:
  ...
  - unchanged waiting/returned-ready fixture counts
- Runtime smoke should remain unchanged because this is an MLPE field-trial validator patch only.

## Next Decision
- Continue with the remaining MLPE field-trial user-filled inputs.
- The likely next pair is returned-capture evidence resolver or final-label validator, depending on whether we prioritize capture evidence handoff or final label intake.
