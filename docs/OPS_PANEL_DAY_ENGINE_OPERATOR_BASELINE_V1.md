...
- 여기에 더해 secondary discovery value panel -> cluster rollup -> attention+discovery preview -> cluster delta까지 붙이더라도 baseline attention 자체를 흔들지 않고 side-by-side supplemental layer로 packaging할 수 있는 상태가 됐다.
...
`build_panel_day_engine_operator_baseline_v1.py` 는 아래 일곱 builder를 순서대로 실행한다.
...
- cluster delta는 마지막에 current cluster rollup과 previous cluster snapshot을 비교해 operator가 reread 없이 바뀐 cluster만 볼 수 있게 한다.
...
3. digest 실행
4. secondary discovery / cluster rollup / preview / cluster delta stack 실행
5. digest-aware + discovery-preview-aware + discovery-delta-aware final baseline manifest/summary로 overwrite
순서로 동작한다.
...
## 왜 cluster preview와 cluster delta를 같이 package하는가
...
- 그래서 baseline orchestration은 이제 primary attention stack뿐 아니라 supplemental discovery cluster preview와 discovery cluster delta stack도 함께 재생성한다.
...
- 이번 확장도 baseline attention 규칙을 바꾸는 것이 아니라, 이미 승인된 discovery preview/delta stack을 baseline rebuild 시 함께 갱신하도록 묶는 packaging change다.
