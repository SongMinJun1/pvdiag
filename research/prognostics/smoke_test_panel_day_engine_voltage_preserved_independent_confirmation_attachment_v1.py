...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="voltage_preserved_independent_confirmation_smoke_") as tmpdir:
        ...
        artifact_payload = json.loads(
            (out_dir / "panel_day_engine_voltage_preserved_independent_confirmation_attachment_v1.json").read_text(
                encoding="utf-8"
            )
        )
        note = (out_dir / "panel_day_engine_voltage_preserved_independent_confirmation_note_v1.md").read_text(
            encoding="utf-8"
        )
        assert_true(
            artifact_payload["input_resolution_sources"]["gap_review_input"] == "explicit_cli",
            artifact_payload,
        )
        ...
        gap_csv = gap_dir / "panel_day_engine_voltage_preserved_confirmation_gap_review_v1.csv"
        manifest_path = tmp / "independent_confirmation_inputs.json"
        manifest_path.write_text(
            json.dumps({"inputs": {"gap_review_input": str(gap_csv)}}, ensure_ascii=False, indent=2) + "\n",
            encoding="utf-8",
        )
        ...
        manifest_payload = json.loads(
            (manifest_out / "panel_day_engine_voltage_preserved_independent_confirmation_attachment_v1.json").read_text(
                encoding="utf-8"
            )
        )
        manifest_note = (manifest_out / "panel_day_engine_voltage_preserved_independent_confirmation_note_v1.md").read_text(
            encoding="utf-8"
        )
        manifest_attachment = pd.read_csv(
            manifest_out / "panel_day_engine_voltage_preserved_independent_confirmation_attachment_v1.csv",
            encoding="utf-8-sig",
        )
        assert_true(manifest_payload["attachment_rows"] == artifact_payload["attachment_rows"], manifest_payload)
        assert_true(
            manifest_payload["truth_intake_ready_rows"] == artifact_payload["truth_intake_ready_rows"],
            manifest_payload,
        )
        assert_true(
            manifest_attachment["independent_confirmation_status"].tolist()
            == attachment["independent_confirmation_status"].tolist(),
            manifest_attachment,
        )
        assert_true(
            manifest_payload["input_resolution_sources"]["gap_review_input"] == "input_manifest",
            manifest_payload,
        )
        ...
        bad_manifest_path = tmp / "bad_independent_confirmation_inputs.json"
        bad_manifest_path.write_text(
            json.dumps(
                {"inputs": {"gap_review_input": str(tmp / "missing_gap_review.csv")}},
                ensure_ascii=False,
                indent=2,
            )
            + "\n",
        ...
        override_payload = json.loads(
            (override_out / "panel_day_engine_voltage_preserved_independent_confirmation_attachment_v1.json").read_text(
                encoding="utf-8"
            )
        )
        assert_true(override_payload["input_resolution_sources"]["gap_review_input"] == "explicit_cli", override_payload)

        missing_key_manifest_path = tmp / "missing_key_independent_confirmation_inputs.json"
        ...
        assert_true(
            "missing `gap_review_input`" in (missing_key_proc.stderr + missing_key_proc.stdout),
