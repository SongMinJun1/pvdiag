...
def build_cleanup_lanes() -> pd.DataFrame:
    rows = [
        {
            "cleanup_lane": "main_dirty_disentangle",
            "current_status": "needs_split",
            "why_now_ko": "main worktree에 docs/release/research/pv_ae dirty가 한꺼번에 섞여 있다.",
            "immediate_action_ko": "current clean branch를 기준으로 main dirty를 lane별로 재분리하는 기준을 먼저 고정한다.",
            "blocking_scope_ko": "main에서 직접 새 작업을 시작할 때 범위 혼선이 생긴다.",
        },
        {
            "cleanup_lane": "source_release_finaldelivery_mirror_policy",
            "current_status": "needs_policy_lock",
            "why_now_ko": "source, runtime package, final_delivery mirror가 따로 움직여 같은 의미가 세 surface에 분산된다.",
            "immediate_action_ko": "어떤 층이 source of truth이고 어떤 층이 generated mirror인지 lane별 정책을 잠근다.",
            "blocking_scope_ko": "한 번 수정할 때 어디까지 동기화해야 하는지 흔들린다.",
        },
        {
            "cleanup_lane": "audit_builder_registry",
            ...
            "immediate_action_ko": "active/current builder와 archival helper를 구분하는 registry를 만든다.",
            "blocking_scope_ko": "비슷한 이름의 script가 많아 다음 작업 진입점이 불분명해진다.",
        },
        {
            "cleanup_lane": "historical_tmp_archive",
            "current_status": "documented_not_reindexed",
            "why_now_ko": "historical BR temp roots가 docs에 남아 있지만 current manifest에는 안 묶여 있다.",
            "immediate_action_ko": "historical archive manifest를 별도 lane으로 둔다.",
            "blocking_scope_ko": "과거 validation/packet 결과를 다시 찾을 때 경로 회상이 필요하다.",
        },
        {
            "cleanup_lane": "runtime_bundle_hygiene",
            "current_status": "heavy_bundle",
            "why_now_ko": "windows runtime bundle 파일 수가 많고 LFS/binary 관리축이 별도로 존재한다.",
            "immediate_action_ko": "runtime bundle과 code/doc lane을 분리해 본다.",
            "blocking_scope_ko": "release 관리와 코드 리뷰 범위가 쉽게 섞인다.",
        },
        {
            "cleanup_lane": "workspace_clutter",
            "current_status": "needs_boundary_cleanup",
