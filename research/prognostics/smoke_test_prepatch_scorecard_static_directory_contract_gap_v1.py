...
def main() -> None:
    ...
    builder = repo_root / "research/prognostics/build_prepatch_scorecard_static_directory_contract_gap_v1.py"
    with tempfile.TemporaryDirectory(prefix="prepatch_scorecard_static_directory_contract_gap_") as tmpdir:
        ...
        payload = json.loads(
            (output_dir / "prepatch_scorecard_static_directory_contract_gap_v1.json").read_text(
                encoding="utf-8"
            )
        )
        detail = pd.read_csv(output_dir / "prepatch_scorecard_static_directory_contract_gap_v1.csv")
        summary = pd.read_csv(output_dir / "prepatch_scorecard_static_directory_contract_gap_summary_v1.csv")
        note = (output_dir / "prepatch_scorecard_static_directory_contract_gap_note_v1.md").read_text(
            encoding="utf-8"
        )

        assert_true(payload["prepatch_directory_rows"] == EXPECTED_ROWS, payload)
