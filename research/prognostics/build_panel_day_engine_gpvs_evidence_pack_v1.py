...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Aggregate GPVS evidence for current real fault panels."
    )
    parser.add_argument(
        "--root",
        type=Path,
...
def canonicalize_gpvs_code(value: object) -> str:
    match = re.match(r"^(F[0-7])", normalize_text(value))
...
def lookup_map(df: pd.DataFrame) -> dict[tuple[str, str], dict[str, object]]:
    ...
    for row in df.to_dict(orient="records"):
        result[as_key(row.get("site"), row.get("panel_id"))] = row
    return result
...
def build_canonical_dictionary_map(df: pd.DataFrame) -> dict[str, dict[str, str]]:
    mapping = {code: payload.copy() for code, payload in DEFAULT_CANONICAL_DICTIONARY.items()}
...
def build_matching_table_map(df: pd.DataFrame) -> dict[str, list[dict[str, object]]]:
    ...
    for row in df.to_dict(orient="records"):
        ...
        if not code:
            continue
        mapping.setdefault(code, []).append(row)
    return mapping
...
def build_internal_evidence(
    ...
    if family_row:
        parts = []
        for key, label in [
            ("prediction_source", "family evaluator source"),
            ("fallback_rule_used", "fallback"),
            ("error_type", "error"),
            ("pred_fault_family", "pred"),
            ("vendor_fault_family", "vendor"),
        ]:
            text = normalize_text(family_row.get(key))
            if text:
...
def map_usefulness_to_compatibility(usefulness: str, summary_final: str) -> str:
    if usefulness == "비권장":
...
def build_compatibility_evidence(
    ...
    if agreement_row:
        for key, label in [
            ...
            ("feature_shift_bucket_ko", "feature_shift"),
            ("overall_gpvs_reference_usefulness_ko", "panel_usefulness"),
        ]:
            text = normalize_text(agreement_row.get(key))
            if text:
                parts.append(f"{label}={text}")
    ...
    for key in ["strong_shift_panel_count", "scenario_conflict_count"]:
        text = normalize_text(compatibility_summary_row.get(key))
        if text:
            parts.append(f"{key}={text}")
    summary_note = normalize_text(compatibility_summary_row.get("note_ko"))
    if summary_note:
        parts.append(summary_note)
    return "; ".join(parts) if parts else "호환성 근거 미확인"
...
def build_matching_evidence(
    ...
    support = normalize_text(canonical_map.get(canonical_code, {}).get("current_real_fault_support_count"))
    if rule:
        parts.append(rule)
    if support:
        parts.append(f"real fault support {support}건")
    if note:
        parts.append(note)
    ...
    if rows:
        match_compact = []
        for row in rows:
            fault = normalize_text(row.get("mlpe_official_fault_ko"))
            role = normalize_text(row.get("match_role_ko"))
            if fault and role:
                match_compact.append(f"{fault}:{role}")
        if match_compact:
            parts.append("matching=" + " / ".join(match_compact))
    if not panel_agreement_df.empty and "overall_gpvs_reference_usefulness_ko" in panel_agreement_df.columns:
        code_panels = panel_agreement_df[
            panel_agreement_df["__canonical_code__"] == canonical_code
        ]
        useful = int((code_panels["overall_gpvs_reference_usefulness_ko"] == "참고가능").sum())
        caution = int((code_panels["overall_gpvs_reference_usefulness_ko"] == "주의참고").sum())
        notrec = int((code_panels["overall_gpvs_reference_usefulness_ko"] == "비권장").sum())
        parts.append(f"panel agreement useful/caution/notrec={useful}/{caution}/{notrec}")
...
def combine_final_recommendation(
    ...
    if matching_policy == "핵심참조" and compatibility_judgment in reference_only_states:
        return "핵심참조"
    if matching_policy == "핵심참조후보" and compatibility_judgment in reference_only_states:
        return "핵심참조후보"
    if matching_policy == "보조참조" and compatibility_judgment in reference_only_states:
        return "보조참조"
    return "비권장"
...
def build_recommendation_reason(
    ...
    label = external_pattern or canonical_code or "GPVS reference"
    if final_recommendation == "핵심참조":
        return f"{label}는 direct root-cause가 아니라 reference-only 핵심참조로만 사용한다."
    if final_recommendation == "핵심참조후보":
        return f"{label}는 reference-only 범위에서 핵심참조후보로만 유지한다."
    if final_recommendation == "보조참조":
        return f"{label}는 직접 root-cause 판정에는 쓰지 말고 보조참조로만 사용한다."
    if final_recommendation == "교란플래그":
        return f"{label}는 고장 판정보다 교란 이벤트 플래그로만 사용한다."
    if final_recommendation == "시스템보류":
        return f"{label}는 현재 패널 단독표보다 시스템/통합 결과표 후보축으로만 보류한다."
    if final_recommendation == "기준선":
        return f"{label}는 fault명이 아니라 기준선 reference로만 사용한다."
    if not matching_policy or matching_policy == "비권장":
        return f"{label}는 현재 matching policy 자체가 비권장이라 direct root-cause로도 reference로도 쓰지 않는다."
    if compatibility_judgment == "비교곤란":
        return f"{label}는 현재 근거가 비교곤란 수준이라 안전하게 비권장으로 둔다."
    return f"{label}는 필요한 근거가 부족하거나 unusable 상태라 비권장으로 둔다."
...
def main() -> None:
    ...
    detailed_df["__canonical_code__"] = detailed_df["gpvs_detailed_top1_fault_type"].map(canonicalize_gpvs_code)
    panel_agreement_df = panel_agreement_df.copy()
    if not panel_agreement_df.empty:
        agreement_external_col = first_existing_column(
            panel_agreement_df,
            ["GPVS_외부참조시나리오명_ko", "GPVS_외부참조패턴_ko"],
        )
        if agreement_external_col:
            panel_agreement_df["__canonical_code__"] = panel_agreement_df[agreement_external_col].map(
                lambda value: ""
            )
        detail_map_for_agreement = lookup_map(detailed_df)
        panel_agreement_df["__canonical_code__"] = panel_agreement_df.apply(
            lambda row: normalize_text(
                (detail_map_for_agreement.get(as_key(row.get("site"), row.get("panel_id"))) or {}).get(
                    "__canonical_code__"
                )
            ),
            axis=1,
    ...
    for row in fault_df.to_dict(orient="records"):
        ...
        canonical_code = canonicalize_gpvs_code((detailed_row or {}).get("gpvs_detailed_top1_fault_type"))
        ...
        matching_evidence = build_matching_evidence(
            canonical_code,
            canonical_map,
            matching_table_map,
            panel_agreement_df,
        ...
        pack_rows.append(
            {
                ...
                "GPVS_매칭근거_ko": matching_evidence,
    ...
    current_code_counts = {
        code: int((detailed_df["__canonical_code__"] == code).sum())
    ...
    note_lines = [
        ...
        "- 비권장은 matching policy 자체가 비권장이거나 required evidence 가 비어 있을 때만 사용합니다.",
        "",
        ...
        + ", ".join(f"{code}={count}" for code, count in current_code_counts.items() if count > 0)
        + " 입니다.",
        "",
        "# 5. 현재 운영 원칙",
