...
V1_SCORES_NAME = "panel_day_engine_run_ranker_v1_scores.csv"
FOLD_SCORES_OUTPUT_NAME = "panel_day_engine_run_ranker_v1_holdout_fold_scores.csv"
SUMMARY_OUTPUT_NAME = "panel_day_engine_run_ranker_v1_holdout_summary.csv"
TOPK_OUTPUT_NAME = "panel_day_engine_run_ranker_v1_holdout_topk_yield.csv"
...
REQUIRED_V1_SCORE_COLS = [*KEY_COLS, "logistic_v1_score"]
SCORE_NAMES = [
    "logistic_v1_holdout",
...
TOPK_COLS = [
    ...
    "top10_or_20_positive_minus_negative",
]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate whether run_ranker_v1 logistic scoring beats v0 references under holdout."
    )
    parser.add_argument(
        "--root",
        type=Path,
...
def load_v0_scores(root: Path) -> pd.DataFrame:
    ...
    for col in ["electrical_core_score", "electrical_core_minus_broadshape_050"]:
        df[col] = pd.to_numeric(df[col], errors="coerce")
    df = df.loc[:, REQUIRED_V0_SCORE_COLS].copy()
    return df.drop_duplicates(subset=KEY_COLS, keep="first").reset_index(drop=True)


def load_v1_scores(root: Path) -> pd.DataFrame:
    path = root / "_share" / V1_SCORES_NAME
    df = read_csv(path)
    ensure_columns(df, REQUIRED_V1_SCORE_COLS, path.name)
    df = drop_repeated_header_rows(df).copy()
    for col in KEY_COLS:
        normalizer = normalize_date if col in {"run_start_date", "run_end_date"} else normalize_text
        df[col] = df[col].map(normalizer)
    df["logistic_v1_score"] = pd.to_numeric(df["logistic_v1_score"], errors="coerce")
    df = df.loc[:, REQUIRED_V1_SCORE_COLS].copy()
    return df.drop_duplicates(subset=KEY_COLS, keep="first").reset_index(drop=True)
...
def prepare_universe(root: Path) -> pd.DataFrame:
    ...
    v1_scores = load_v1_scores(root).rename(columns={"logistic_v1_score": "prototype_logistic_v1_score"})
    merged = feature_df.merge(v0_scores, on=KEY_COLS, how="left", validate="one_to_one")
    merged = merged.merge(v1_scores, on=KEY_COLS, how="left", validate="one_to_one")
    missing_cols = [
        col
        for col in ["electrical_core_score", "electrical_core_minus_broadshape_050", "prototype_logistic_v1_score"]
        if merged[col].isna().any()
    ]
    if missing_cols:
        raise SystemExit(f"merged run universe missing reference scores: {missing_cols}")
    merged["evaluation_group"] = merged["cohort_hint"].map(evaluation_group)
...
def compute_holdout_metrics(
    ...
    for top_k in TOP_K_VALUES:
        ...
        metric = positive_rate - negative_rate
        top_stats[top_k] = {
            "positive_like": counts["positive_like"],
            "negative_like": counts["negative_like"],
            "monitor_like": counts["monitor_like"],
            "unlabeled_other": counts["unlabeled_other"],
            "positive_minus_negative": metric,
        }
        topk_rows.append(
            {
                ...
                "top10_or_20_positive_minus_negative": metric,
            }
        )
...
def evaluate_fold(universe: pd.DataFrame, spec: dict[str, object]) -> tuple[list[dict[str, object]], list[dict[str, object]]]:
    ...
    train_labeled = train_df.loc[train_df["evaluation_group"].isin(LABELED_GROUPS)].copy()
    train_positive_count = int(train_labeled["evaluation_group"].eq("positive_like").sum())
    train_negative_count = int(train_labeled["evaluation_group"].eq("negative_like").sum())
    test_counts = compute_group_counts(test_df) if not test_df.empty else compute_group_counts(universe.iloc[0:0])
    ...
    y_train = train_labeled["evaluation_group"].eq("positive_like").astype(int)
    ...
    scored_test["logistic_v1_holdout"] = logistic.predict_proba(scaled_test)[:, 1]
...
def build_summary(fold_scores: pd.DataFrame) -> pd.DataFrame:
    if fold_scores.empty:
        return pd.DataFrame(columns=SUMMARY_COLS)

    rows: list[dict[str, object]] = []
    all_fold_counts = (
        fold_scores[["fold_type", "fold_id"]]
        .drop_duplicates()
        .groupby("fold_type", dropna=False)
        .size()
        .to_dict()
    )
    valid = fold_scores.loc[fold_scores["skip_reason"].eq("")].copy()
    ...
    for (score_name, fold_type), group in fold_scores.groupby(["score_name", "fold_type"], dropna=False):
        ...
        note = f"valid_folds={len(valid_group)}/{total_folds}"
        if score_name == "logistic_v1_holdout":
            refs = valid.loc[
                valid["fold_type"].eq(fold_type)
                & valid["score_name"].isin({"electrical_core_score", "electrical_core_minus_broadshape_050"})
            ].copy()
            if not refs.empty:
                best_ref = float(refs.groupby("score_name")["top20_positive_minus_negative"].mean().max())
                logistic_mean = float(valid_group["top20_positive_minus_negative"].mean())
                delta = logistic_mean - best_ref
                direction = "beats_best_v0_by" if delta > 0 else "below_or_ties_best_v0_by"
                note = f"{note};{direction}={delta:.4f}"
        else:
            note = f"{note};v0_reference"
        ...
        rows.append(
            {
                ...
                "mean_top10_positive_minus_negative": valid_group["top10_positive_minus_negative"].mean(),
                "mean_top20_positive_minus_negative": valid_group["top20_positive_minus_negative"].mean(),
...
def main() -> None:
    ...
    summary = build_summary(fold_scores)
