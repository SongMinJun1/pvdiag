...
def main() -> None:
    ...
    builder = repo_root / "research/prognostics/build_voltage_preserved_static_directory_contract_gap_v1.py"
    with tempfile.TemporaryDirectory(prefix="voltage_preserved_static_directory_contract_gap_") as tmpdir:
        ...
        payload = json.loads(
            (output_dir / "voltage_preserved_static_directory_contract_gap_v1.json").read_text(
                encoding="utf-8"
            )
        )
        detail = pd.read_csv(output_dir / "voltage_preserved_static_directory_contract_gap_v1.csv")
        summary = pd.read_csv(output_dir / "voltage_preserved_static_directory_contract_gap_summary_v1.csv")
        note = (output_dir / "voltage_preserved_static_directory_contract_gap_note_v1.md").read_text(
            encoding="utf-8"
        )

        assert_true(payload["voltage_directory_rows"] == EXPECTED_ROWS, payload)
        ...
        assert_true(
            payload["voltage_directory_rows"]
            == int(summary[summary["key"].eq("voltage_directory_rows")]["count"].iloc[0]),
            payload,
        )
        assert_true("fully contract-closed" in note, note)

    print("smoke ok: voltage_preserved_static_directory_contract_gap_v1")
