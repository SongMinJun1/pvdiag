...
def main() -> None:
    ...
    assert_true(manifest_payload["payload_container"] == "readable_comment_blocks", manifest_payload)
    ...
    assert_true("EMBEDDED_TEXT_JSON_CHUNKS" not in single_text, single)
    assert_true("# pvdiag_payload_file " in single_text, single)
    assert_true("#|" in single_text, single)
    assert_true("PAYLOAD_FILE_INDEX" in single_text, single)
    assert_true("PAYLOAD_STRUCTURE_NOTE" in single_text, single)
    assert_true("# region Embedded readable source payload" in single_text, single)
    assert_true("# endregion" in single_text, single)
    assert_true(len(single_text.splitlines()) < 15_000, single)
