...
def main() -> None:
    ...
    builder = repo_root / "research/prognostics/build_static_artifact_reference_contract_gap_v1.py"
    with tempfile.TemporaryDirectory(prefix="static_artifact_reference_contract_gap_") as tmpdir:
        ...
        payload = json.loads(
            (output_dir / "static_artifact_reference_contract_gap_v1.json").read_text(
                encoding="utf-8"
            )
        )
        detail = pd.read_csv(output_dir / "static_artifact_reference_contract_gap_v1.csv")
        summary = pd.read_csv(output_dir / "static_artifact_reference_contract_gap_summary_v1.csv")
        note = (output_dir / "static_artifact_reference_contract_gap_note_v1.md").read_text(
            encoding="utf-8"
        )

        assert_true(payload["static_artifact_rows"] == EXPECTED_ROWS, payload)
