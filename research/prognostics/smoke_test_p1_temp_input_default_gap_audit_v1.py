...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="p1_temp_input_default_gap_audit_") as tmpdir:
        ...
        assert_true(payload["non_mlpe_manifest_or_explicit_closed_rows"] == 8, payload)
        assert_true(payload["explicit_cli_only_open_rows"] == 0, payload)
        ...
        assert_true(payload["open_gap_files"] == [], payload)
        for key, count in EXPECTED_WORKFLOW_COUNTS.items():
            assert_true(payload["workflow_lane_counts"][key] == count, payload)

        closed_rows = detail[detail["relative_path"].eq(EXPECTED_OPEN_GAP_FILE)]
        assert_true(len(closed_rows) == 1, detail.to_dict("records"))
        closed_row = closed_rows.iloc[0]
        assert_true(closed_row["closure_status"] == "closed", closed_row.to_dict())
        assert_true(closed_row["default_constant"] == "DEFAULT_RUNTIME_ROOT", closed_row.to_dict())
        assert_true(closed_row["explicit_cli_flag"] == "--runtime-root", closed_row.to_dict())
        assert_true(int(closed_row["has_explicit_cli_arg"]) == 1, closed_row.to_dict())
        assert_true(int(closed_row["has_input_manifest_arg"]) == 1, closed_row.to_dict())
        assert_true(int(closed_row["has_source_specific_manifest_resolver"]) == 1, closed_row.to_dict())
        assert_true("prior explicit-CLI-only runtime-root gap is now closed" in note, note)
        assert_true(
