...
OWNER_BRANCH = "BR-20260429-217"
EXPECTED_VOLTAGE_DIRECTORY_ROWS = 4
TARGET_WORKFLOW_LANE = "panel_engine_voltage_preserved"

DETAIL_OUTPUT_NAME = "voltage_preserved_static_directory_contract_gap_v1.csv"
SUMMARY_OUTPUT_NAME = "voltage_preserved_static_directory_contract_gap_summary_v1.csv"
NOTE_OUTPUT_NAME = "voltage_preserved_static_directory_contract_gap_note_v1.md"
JSON_OUTPUT_NAME = "voltage_preserved_static_directory_contract_gap_v1.json"
...
EXPECTED_CONTRACTS = {
    ...
    (
        "research/prognostics/build_panel_day_engine_voltage_preserved_positive_search_v1.py",
        "panel_day_engine_durable_hold_raw_shape_review_br091_check",
    ): (["hold_input"], ["--hold-input"]),
    (
        "research/prognostics/build_panel_day_engine_voltage_preserved_confirmation_packet_v1.py",
        "panel_day_engine_voltage_preserved_positive_search_br092_check",
    ): (["candidate_input"], ["--candidate-input"]),
    (
        "research/prognostics/build_panel_day_engine_voltage_preserved_raw_source_attachment_v1.py",
        "panel_day_engine_voltage_preserved_confirmation_packet_br093_check",
    ): (["source_map_input"], ["--source-map-input"]),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Identify which voltage-preserved static directory references already have "
...
def build_gap(repo_root: Path, max_file_bytes: int) -> list[dict[str, object]]:
    ...
    for idx, row in enumerate(voltage_rows, start=1):
...
def summary_payload(gap_rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    contract_complete = int(
        len(gap_rows) == EXPECTED_VOLTAGE_DIRECTORY_ROWS
        and fail_count == 0
    ...
    payload: dict[str, object] = {
        ...
        "expected_voltage_directory_rows": EXPECTED_VOLTAGE_DIRECTORY_ROWS,
        "voltage_directory_rows": len(gap_rows),
        ...
        "recommended_next_branch": (
            "voltage_preserved_static_directory_contract_closed_no_rewrite"
            if contract_complete
            else "patch_voltage_preserved_static_directory_gap_scripts_with_input_manifest_resolver"
        ),
    }
    return payload
...
def summary_rows(payload: dict[str, object]) -> list[dict[str, object]]:
    keys = [
        "expected_voltage_directory_rows",
        "voltage_directory_rows",
        "source_file_count",
...
def write_note(output_dir: Path, payload: dict[str, object]) -> Path:
    ...
    decision_lines = (
        [
            "- Voltage-preserved static directory references are fully contract-closed.",
            "- No default rewrite is authorized by this audit.",
            "- Keep direct runtime semantic and bulk rewrite permission at 0.",
        ]
        if contract_complete
        else [
            "- Voltage-preserved static directory references are not fully contract-closed yet.",
            "- The next code patch should add input-manifest and manifest resolver handling before any path rewrite.",
            "- Keep direct runtime semantic and bulk rewrite permission at 0.",
        ]
    )
    lines = [
        "# Voltage-Preserved Static Directory Contract Gap",
        "",
        "## Boundary",
        "- This branch identifies voltage-preserved input-contract gaps only.",
        ...
        "",
        "## Summary",
        f"- expected voltage directory rows: `{payload['expected_voltage_directory_rows']}`",
        f"- observed voltage directory rows: `{payload['voltage_directory_rows']}`",
