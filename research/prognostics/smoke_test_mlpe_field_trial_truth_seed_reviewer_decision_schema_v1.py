...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="mlpe_truth_seed_decision_schema_smoke_") as tmpdir:
        tmp = Path(tmpdir)
        packet = tmp / "packet.csv"
        schema_dir = tmp / "schema"
        label_input = tmp / "real_labels.csv"
        runbook_dir = tmp / "runbook"
        review_packet_dir = tmp / "truth_seed_review_packet"
        output_dir = tmp / "decision_schema"
        ...
        assert_true(packet_proc.returncode == 0, packet_proc.stderr or packet_proc.stdout)
        ...
        proc = run(
            [
                ...
                str(repo_root),
                "--packet",
                str(review_packet_dir / "mlpe_field_trial_truth_seed_review_packet_v1.csv"),
                "--output-dir",
                str(output_dir),
            ],
            repo_root,
        ...
        assert_true(payload["template_rows"] == 2, payload)
        assert_true(payload["schema_rows"] == 25, payload)
        assert_true(payload["allowed_value_rows"] == 27, payload)
        assert_true(payload["reviewer_decision_attached_rows"] == 0, payload)
        ...
        print(json.dumps({"smoke": "ok", "template_rows": 2, "allowed_value_rows": 27}, ensure_ascii=False))
