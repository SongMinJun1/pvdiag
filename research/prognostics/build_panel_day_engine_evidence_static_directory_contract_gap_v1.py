...
from build_repo_static_directory_reference_inventory_v1 import build_inventory


OWNER_BRANCH = "BR-20260429-218"
EXPECTED_EVIDENCE_DIRECTORY_ROWS = 20
TARGET_WORKFLOW_LANE = "panel_day_engine_evidence"

DETAIL_OUTPUT_NAME = "panel_day_engine_evidence_static_directory_contract_gap_v1.csv"
SUMMARY_OUTPUT_NAME = "panel_day_engine_evidence_static_directory_contract_gap_summary_v1.csv"
NOTE_OUTPUT_NAME = "panel_day_engine_evidence_static_directory_contract_gap_note_v1.md"
JSON_OUTPUT_NAME = "panel_day_engine_evidence_static_directory_contract_gap_v1.json"
...
EXPECTED_CONTRACTS = {
    (
        ...
        19,
        "panel_day_engine_algorithm_evolution_map_br079_check",
    ): (["br079_root"], ["--br079-root"]),
    (
        "research/prognostics/build_panel_day_engine_direction_assumption_audit_v1.py",
        20,
        "panel_day_engine_subtype_truth_expansion_backlog_br080_check",
    ): (["br080_root"], ["--br080-root"]),
    (
        "research/prognostics/build_panel_day_engine_direction_assumption_audit_v1.py",
        21,
        "panel_day_engine_episode_truth_map_br081_check",
    ): (["br081_root"], ["--br081-root"]),
    (
        "research/prognostics/build_panel_day_engine_direction_assumption_audit_v1.py",
        22,
        "panel_day_engine_episode_truth_review_packet_br082_check",
    ): (["br082_root"], ["--br082-root"]),
    (
        "research/prognostics/build_panel_day_engine_durable_hold_raw_shape_review_v1.py",
        20,
        "panel_day_engine_episode_truth_durable_shape_review_br089_check",
    ): (["shape_input"], ["--shape-input"]),
    (
        "research/prognostics/build_panel_day_engine_fault_family_judgment_candidate_packet_v1.py",
        14,
        "cross_axis_manifest_sync_review_check",
    ): (["cross_axis_input"], ["--cross-axis-input"]),
    (
        "research/prognostics/build_panel_day_engine_fault_family_judgment_candidate_packet_v1.py",
        18,
        "fault_family_regression_pressure_packet_check",
    ): (["pressure_input"], ["--pressure-input"]),
    (
    ...
    (
        ...
        15,
        "raw_waveform_physical_support_review_check",
    ): (["raw_review_input"], ["--raw-review-input"]),
    (
        "research/prognostics/build_panel_day_engine_physical_evidence_request_packet_v1.py",
        14,
        "physical_confirmation_requirements_review_check",
    ...
    (
        ...
        14,
        "voltage_dominant_physical_vs_artifact_review_check",
    ...
    (
        "research/prognostics/build_panel_day_engine_subtype_threshold_replay_pilot_v1.py",
        25,
        "panel_day_engine_reviewed_episode_truth_rows_br089_mixed_check",
    ): (["reviewed_truth_input"], ["--reviewed-truth-input"]),
    (
        "research/prognostics/build_panel_day_engine_subtype_truth_expansion_backlog_v1.py",
        22,
        "panel_day_engine_algorithm_evolution_map_br079_check",
    ): (["br079_gap_input"], ["--br079-gap-input"]),
    (
        "research/prognostics/build_panel_day_engine_subtype_truth_expansion_backlog_v1.py",
        27,
        "fault_family_judgment_candidate_packet_check",
    ): (["candidate_packet_input"], ["--candidate-packet-input"]),
    (
        "research/prognostics/build_panel_day_engine_subtype_truth_expansion_backlog_v1.py",
        31,
        "local_morphology_family_shape_review_check",
    ): (["shape_review_input"], ["--shape-review-input"]),
    (
        "research/prognostics/build_panel_day_engine_subtype_truth_expansion_backlog_v1.py",
        35,
        "physical_confirmation_requirements_review_check",
    ): (["physical_confirmation_input"], ["--physical-confirmation-input"]),
    (
        "research/prognostics/build_panel_day_engine_subtype_truth_expansion_backlog_v1.py",
        39,
        "common_cause_exact_seed_search_check",
    ): (["common_cause_search_input"], ["--common-cause-search-input"]),
    (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Identify which panel-day evidence static directory references already have "
...
def build_gap(repo_root: Path, max_file_bytes: int) -> list[dict[str, object]]:
    ...
    for idx, row in enumerate(evidence_rows, start=1):
        ...
        expected_manifest_keys, explicit_cli_flags = EXPECTED_CONTRACTS.get(
            (source_file, line_no, directory_slug),
            ([], []),
        )
...
def summary_payload(gap_rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    contract_complete = int(
        len(gap_rows) == EXPECTED_EVIDENCE_DIRECTORY_ROWS
        and fail_count == 0
    ...
    payload: dict[str, object] = {
        ...
        "expected_evidence_directory_rows": EXPECTED_EVIDENCE_DIRECTORY_ROWS,
        "evidence_directory_rows": len(gap_rows),
        ...
        "recommended_next_branch": (
            "panel_day_engine_evidence_static_directory_contract_closed_no_rewrite"
            if contract_complete
            else "patch_panel_day_engine_evidence_static_directory_gap_scripts_with_input_manifest_resolver"
        ),
    }
    return payload
...
def summary_rows(payload: dict[str, object]) -> list[dict[str, object]]:
    keys = [
        "expected_evidence_directory_rows",
        "evidence_directory_rows",
        "source_file_count",
...
def write_note(output_dir: Path, payload: dict[str, object]) -> Path:
    ...
    decision_lines = (
        [
            "- Panel-day evidence static directory references are fully contract-closed.",
            "- No default rewrite is authorized by this audit.",
            "- Keep direct runtime semantic and bulk rewrite permission at 0.",
        ]
        if contract_complete
        else [
            "- Panel-day evidence static directory references are not fully contract-closed yet.",
            "- The next code patch should add input-manifest and manifest resolver handling before any path rewrite.",
            "- Keep direct runtime semantic and bulk rewrite permission at 0.",
        ]
    )
    lines = [
        "# Panel-Day Evidence Static Directory Contract Gap",
        "",
        "## Boundary",
        "- This branch identifies panel-day evidence input-contract gaps only.",
        ...
        "",
        "## Summary",
        f"- expected evidence directory rows: `{payload['expected_evidence_directory_rows']}`",
        f"- observed evidence directory rows: `{payload['evidence_directory_rows']}`",
